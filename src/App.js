...
const TableView = ({ tableData }) => {
    if (!tableData || !tableData.headers || !tableData.rows || tableData.rows.length === 0) {
        return <p className="p-4 text-sm italic text-gray-500 dark:text-gray-400">The AI returned an empty or invalid table.</p>;
    }
                ...
                <footer className="p-4">
                    <div className="max-w-3xl mx-auto">
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-2 flex items-center">
                            ...
                            <button onClick={() => handleSendMessage(input)} className="p-3 rounded-full bg-blue-600 text-white hover:bg-blue-700 disabled:bg-blue-300 disabled:cursor-not-allowed transition-colors" disabled={!input.trim() || isLoading}><Send size={24} /></button>
                        </div>
                    </div>
                </footer>
            </main>
